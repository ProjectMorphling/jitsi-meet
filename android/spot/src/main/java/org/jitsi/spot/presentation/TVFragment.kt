...
class TVFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            val joinCode = it.getString(PAIRING_CODE_KEY)
            joinCode?.let {
                viewModel.pairingCode.set(it)
            }
        }
    }
    ...
    override fun onDestroy() {
        activity?.let {
            (it as ComponentHolder).clearComponent(SpotComponent::class.java)
        }
    ...
    private fun initPort() {
        ...
        port?.setWebMessageCallback(object : WebMessageCallback() {
            override fun onMessage(port: WebMessagePort, message: WebMessage) {
                ...
                MessageHandler.constructMessage(message)?.let {
                    when (it.type) {
    ...
    private fun inject() {
        activity?.let {
            (it as ComponentHolder).getComponent(SpotComponent::class.java, this)?.inject(this)
        }
    }

...
import org.jitsi.spot.domain.MeetingStatus
...
class MessageHandler {
    ...
    companion object {
        ...
        fun constructMessage(webMessage: WebMessage): Message? {
            val webMessageData = WebMessageData(webMessage)

            return when (webMessageData.command) {
                AUDIO_MUTED_STATE_COMMAND -> AudioMutedMessage(webMessageData.args)
                UPDATE_LONG_LIVED_PAIRING_CODE_COMMAND -> UpdateLongLivedPairingCodeMessage(
                    webMessageData.args
                )
                ADJUST_VOLUME_COMMAND -> AdjustVolumeMessage(webMessageData.args)
                RESET_COMMAND -> ResetMessage()
                MEETING_STATUS_COMMAND -> MeetingStatusMessage(webMessageData.args)
                SHORT_JOIN_CODE_COMMAND -> ShortJoinCodeMessage(webMessageData.args)
                else -> null
            }
        }
